...
// read-only from Comment Service's perspective: User/Video Service own writes.
...
export const commentLikes = mysqlTable(
  ...
  (table) => ({
    commentUserUnique: uniqueIndex("minitube_comment_likes_comment_user_unique").on(table.commentId, table.userId),
  })
);
